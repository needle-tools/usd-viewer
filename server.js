...
const { injectClientIpIntoAnalyticsTrackBody, maskIpForLog } = require('./scripts/analytics-proxy')
...
// ---------------------------------------------------------------------------
// Same-origin proxy for analytics.
//
...
// and the analytics endpoint does not send it. Rather than weaken COEP (which would
// break SharedArrayBuffer on Safari), we proxy analytics through this origin so the
// script and its event beacons are same-origin and CORP no longer applies.
//
// The analytics script derives its API base from its own <script src> by splitting
...
const ANALYTICS_ORIGIN = "https://analytics-2.needle.tools";
...
async function proxyToAnalytics(request, reply) {
  // request.url is the full path + query (e.g. "/api/track?...") — forward as-is.
  const target = ANALYTICS_ORIGIN + request.url;

  const headers = {};
  for (const h of ["content-type", "user-agent", "accept", "accept-language"]) {
    if (request.headers[h]) headers[h] = request.headers[h];
  }
  // Preserve the real client IP so analytics attributes / geolocates correctly.
  // X-Forwarded-For is best-effort (Traefik only keeps it when it trusts
  // Cloudflare); the reliable path is the ip_address stamped into the track body
  // below from CF-Connecting-IP.
  ...
  const body = hasBody
    ? injectClientIpIntoAnalyticsTrackBody(request.url, request.body, cfConnectingIp)
    : undefined;
  ...
  // if this disagrees with the analytics dashboard, the IP isn't reaching it)
  ...
  if (request.url.split("?")[0] === "/api/track") {
    console.log("[analytics] track", JSON.stringify({
      cfCountry: request.headers["cf-ipcountry"] || "?",
      ipPrefix: maskIpForLog(cfConnectingIp),
  ...
  try {
    ...
    request.log.warn({ err: err && err.message, target }, "Analytics proxy upstream failed");
...
// regardless of registration order — so feedback never hits the analytics proxy.
